  ...
  return (
    <div className="bg-surface rounded-lg border border-border shadow-card p-4 space-y-4 nums">
      <div className="flex items-center justify-between gap-2">
        <h3 className="font-semibold text-foreground">{t('analysis.title')}</h3>
        <span
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-blue-50 rounded p-3 text-center">
          ...
          <div className="text-xs text-foreground-subtle">{t('analysis.stateCount')}</div>
        </div>
        <div className="bg-green-50 rounded p-3 text-center">
          <div className="text-2xl font-bold text-green-600">{analysis.transition_count}</div>
          <div className="text-xs text-foreground-subtle">{t('analysis.transitionCount')}</div>
        </div>
        <div className="bg-purple-50 rounded p-3 text-center">
          <div className="text-2xl font-bold text-purple-600">{analysis.simulation_run_count}</div>
          <div className="text-xs text-foreground-subtle">{t('analysis.simCount')}</div>
        </div>
        <div className="bg-orange-50 rounded p-3 text-center">
          <div className="text-2xl font-bold text-orange-600">{analysis.terminal_states.length}</div>
          <div className="text-xs text-foreground-subtle">{t('analysis.terminalCount')}</div>
        </div>
      </div>
      ...
      {hasTerminal && (
        <div className="bg-surface-muted border border-border rounded p-3">
          <p className="text-sm font-medium text-foreground-muted">{t('analysis.terminal')}</p>
          <div className="mt-1 flex flex-wrap gap-1">
            {analysis.terminal_states.map(s => (
              <span key={s} className="px-2 py-0.5 bg-surface-muted rounded text-xs text-foreground-muted">{sampleLabel(s, lang)}</span>
            ))}
          </div>
        </div>
      )}
